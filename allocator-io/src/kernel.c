...
kernel_free(void *ptr, size_t size) {
    if (munmap(ptr, size) < 0) failed_kernel_free();
}
...
#endif
    if (madvise(ptr, size, MADV_DONTNEED) < 0) failed_kernel_reset();
}
...
kernel_free(void *ptr, size_t size) {
    (void)size;
    if (VirtualFree(ptr, 0, MEM_RELEASE) == 0) failed_kernel_free();
}
...
kernel_reset(void *ptr, size_t size) {
    ...
    memset(ptr, 0x7e, size);
#endif
    if (VirtualAlloc(ptr, size, MEM_RESET, PAGE_READWRITE) == NULL) failed_kernel_reset();
}
